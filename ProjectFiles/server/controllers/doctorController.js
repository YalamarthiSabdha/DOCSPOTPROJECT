...
exports.doctorSignup = catchAsync(async (req, res, next) => {
  ...
  console.log('Has userId?', !!req.body.userId);
  ...
  if (!req.file) {
    ...
    return next(new AppError("Please upload your MBBS certificate", 400));
  }
...
exports.markAppointmentCompleted = catchAsync(async (req, res, next) => {
  ...
  unseenNotifications.push({
    ...
    message: `Your appointment with ${appointment.doctorInfo.fullName} has been completed. Please rate your experience.`,
    data: {
...
exports.getCompletedAppointments = catchAsync(async (req, res, next) => {
  const appointments = await Appointment.find({
    doctorId: req.params.id,
    isCompleted: true,
  });
